...
contract SuperFantastic is ERC721, ERC721Enumerable, Ownable2Step, ReentrancyGuard {
    ...
    /// @notice Initializes the SuperFantastic contract
    ...
    constructor(
        ...
        ERC721("SuperFantastic Event Collection", "SFEC")
        Ownable(msg.sender)
    {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if (isFlagged[tokenId]) {
            description = string(abi.encodePacked(
                bytes(meta.customText).length > 0 ? meta.customText : "SuperFantastic Event NFT",
                " [Content flagged by moderators]"
            ));
        } else {
            description = bytes(meta.customText).length > 0 
                ? meta.customText 
                : "SuperFantastic Event NFT - A soulbound memory captured in real life!";
        }
        ...
        string memory json = string(abi.encodePacked(
            '{"name":"SuperFantastic #', tokenId.toString()
        ));
